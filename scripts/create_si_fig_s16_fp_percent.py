#!/usr/bin/env python3
"""
Create SI Fig. S16: among-significant FP% = 100 × FP/(TP+FP) for four cohorts.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument(
        "--output-image",
        type=Path,
        default=Path("figures") / "SF16_fp_percent_distribution_cleaned_mean.png",
    )
    ...
    try:
        for title, rel in DEFAULT_COHORTS:
            ...
            print(f"[SF16] {labeled}: collected from {rel.name}")
