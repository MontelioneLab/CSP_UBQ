#!/usr/bin/env python3
"""
SI Fig. S25 — Five selected apo–apo control panels (S25A–S25E).

Copies the precomputed aggregate figures into ``figures/SF25{A–E}_*.png``.
...
def _output_name(letter: str, pair_id: str) -> str:
    return f"SF25{letter}_apo_apo_{pair_id}.png"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="Destination directory for SF25A–E PNGs (default: figures).",
    )
    ...
    for letter, pair_id in PANELS:
        ...
        print(f"[SF25{letter}] Wrote {out} (from {src})")
