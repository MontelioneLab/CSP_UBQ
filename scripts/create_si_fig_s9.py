...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create SI Fig. S9 (HA/CA CSP confusion matrix histograms)."
    )
    parser.add_argument(
        "--outputs-dir",
        type=Path,
    ...
    parser.add_argument("--output-image", type=Path, default=Path("figures") / "SF9_ha_ca.png")
...
def main() -> int:
    ...
    if not eligible:
        print(
            "No targets left after HA/CA table filter; cannot build SI Fig. S9.",
            file=sys.stderr,
        )
        return 1
    ...
    with tempfile.TemporaryDirectory(prefix="si_fig_s10_") as tmp_dir:
        ...
        if not panel_a_path.is_file() or not panel_b_path.is_file():
            print(
                "Cannot compose SI Fig. S9: one or both panel PNGs were not written "
    ...
    print(f"SI Fig. S9 saved to {output_image.resolve()} (n={len(eligible)})")
    return 0
