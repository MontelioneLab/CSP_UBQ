#!/usr/bin/env python3
"""
SI Fig. S16 — ideal N/H offsets (offset grid heatmap from grid search).

Reads grid search CSV files from outputs/, creates the heatmap, and saves to
./figures/SF16_ideal_offsets.png.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create SI Fig. S16 (ideal N/H offsets heatmap).")
    ...
    suppl_path = figures_dir / "SF16_ideal_offsets.png"
    os.rename(heatmap_path, suppl_path)
    print(f"SI Fig. S16 saved to {suppl_path.resolve()}")
    return 0
