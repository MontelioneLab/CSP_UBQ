#!/usr/bin/env python3
"""
SI Table S9 + SI Fig. S8 — targets in CSP_UBQ.csv but not in the buffer-similar
...
Writes:
  ...
  - figures/ST9_dissimilar_apo_holo_conditions.tex (via create_csp_latex_table.py)
  - figures/SF8_dissimilar_apo_holo_conditions.png (two-panel confusion histograms)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="SI Table S9 + SI Fig. S8 (dissimilar apo/holo experimental conditions)."
    )
    parser.add_argument(
        "--full-csp-csv",
        type=Path,
    ...
    parser.add_argument(
        "--output-tex",
        type=Path,
        default=Path("figures") / "ST9_dissimilar_apo_holo_conditions.tex",
    )
    parser.add_argument(
        "--subset-csv",
        type=Path,
