#!/usr/bin/env python3
"""
SI Fig. S15 — Per-target **F1 scores** for 1D H / N / Cα / Hα CSPs
(boxplots + paired Wilcoxon).

The paired cohort is the intersection of targets that yield a valid F1 for
every atom in :data:`SF15_ATOM_ORDER` (H/N/CA/HA) after the CA–shift coverage
gate (default ``data/CSP_UBQ_ph0.5_temp5C.csv`` and ``--min-ca-coverage``), so
*n* is the HA/CA-complete subset (expected 101). Implements SI Fig. S15 via
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SI Fig. S15: 1D H/N/Cα/Hα F1 score boxplots with paired Wilcoxon (Holm-adjusted)."
    )
    parser.add_argument(
        "--outputs-dir",
        type=Path,
