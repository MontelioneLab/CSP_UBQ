...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create SI Fig. S26 (F1 vs MCC scatter plot)."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output-image",
        type=Path,
        default=Path("figures") / "SF26_f1_vs_mcc.png",
        help="Destination for SI Fig. S26 image.",
    )
...
def main() -> int:
    ...
    print(f"SI Fig. S26 saved to {output_image.resolve()}")
    return 0
