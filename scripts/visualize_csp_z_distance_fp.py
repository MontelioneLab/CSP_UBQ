"""
CSP Z-Score vs Distance Heatmap Visualization (False Positives Only)

This script creates a 2D heatmap showing the distribution of False Positive data points
...
False Positives are defined as: significant == 1 AND is_positive == True
where is_positive uses the 'occluded_or_ca_or_interaction' strategy:
...
def is_false_positive(row: dict) -> bool:
    """
    Determine if a row represents a False Positive.
    
    False Positive: significant == 1 AND is_positive == True
    where is_positive uses the 'occluded_or_ca_or_interaction' strategy:
    is_positive = is_occluded OR passes_ca_filter OR is_interacting
    ...
    Returns:
        True if the row is a False Positive, False otherwise
    """
    ...
    # Determine positive status using 'occluded_or_ca_or_interaction' strategy
    is_positive = is_occluded or passes_ca_filter or is_interacting
    
    # False Positive: significant CSP AND is_positive
    return significant and is_positive
...
def collect_data_from_files(csv_files: List[str]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    print(f"Processed {total_rows} total rows, found {fp_rows} False Positives")
...
def create_heatmap(csp_z_values: np.ndarray, distance_values: np.ndarray, 
    ...
    """
    Create and save a 2D heatmap showing the distribution of False Positive data points.
    ...
    ax.set_title('CSP Z-Score vs Minimum CA Distance Heatmap (False Positives Only)', fontsize=14, fontweight='bold')
    ...
    print(f"Total False Positive data points: {len(csp_z_values)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create 2D heatmap of CSP Z-score vs minimum CA distance (False Positives only)"
    )
    parser.add_argument(
        "--outputs",
    ...
    # Collect data from all files (False Positives only)
    print("Reading data from CSV files (False Positives only)...")
    csp_z_values, distance_values = collect_data_from_files(csv_files)
    
    if len(csp_z_values) == 0:
        print("Error: No valid False Positive data points found in any CSV files")
        sys.exit(1)
