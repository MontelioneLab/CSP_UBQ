...
Generator script ``create_si_fig_s11.py`` matches output prefix ``SF11_``.

Reuses existing logic from scripts/analyze_targets_ca.py and writes:
  ./figures/SF11_f1_ca_vs_exclusive.png

Targets are trimmed with the **same CA-shift eligibility rule** as SI Fig. S14 /
...
def main() -> int:
    ...
    if not eligible:
        print(
            "No targets left after CA shift coverage filter; cannot render SI Fig. S11.",
            file=sys.stderr,
        )
        return 1
