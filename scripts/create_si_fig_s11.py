...
SI Fig. S11 — CA-inclusive CSP confusion matrix histograms (two-panel image).

- Panel A: stacked histogram of significant residues (TP/FP only)
- Panel B: stacked confusion-matrix histogram (TN/FP/FN/TP)

Output:
- figures/SF11_ca_inclusive.png

Target eligibility matches SI Fig. S12 / S15: CSV rows are resolved to
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create SI Fig. S11 (CA-inclusive CSP confusion matrix histograms)."
    )
    parser.add_argument(
        "--outputs-dir",
        type=Path,
    ...
    parser.add_argument("--output-image", type=Path, default=Path("figures") / "SF11_ca_inclusive.png")
    parser.add_argument(
        "--min-ca-coverage",
        type=float,
        default=DEFAULT_MIN_CA_SHIFT_ROW_COVERAGE,
        help=(
            "Same as SI Fig. S12 / S15: require strictly more than this fraction of "
...
def main() -> int:
    ...
    if not eligible:
        print(
            "No targets left after CA shift coverage filter; cannot build SI Fig. S11.",
            file=sys.stderr,
        )
        return 1
