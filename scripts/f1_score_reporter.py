...
This script extends the core functionality of analyze_targets.py. Provide either:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Calculate and report F1 scores for a subset of targets. "
            "Use --targets-csv (holo_pdb column) or --targets (comma-separated IDs). "
            "With --targets only, each ID must exist in data/CSP_UBQ.csv."
        )
    )
...
def validate_targets(
    target_list: List[str], validation_csv: Path
) -> tuple[Set[str], List[str]]:
    """
    Validate that CLI-provided targets exist in the study table (used with --targets only).

    Args:
        target_list: List of target PDB IDs to validate
        validation_csv: Path to CSP_UBQ-style CSV listing valid holo_pdb values
...
def main(argv: list[str]) -> int:
    ...
    validation_path = (
        ...
        if DEFAULT_VALIDATION_CSV.is_absolute()
        else repo_root / DEFAULT_VALIDATION_CSV
    )
    ...
    if args.targets_csv is not None:
        ...
        if not targets_csv.is_file():
            ...
            return 1
        try:
            valid_targets = load_holo_pdb_set(targets_csv)
        ...
        if not valid_targets:
            ...
            return 1
        print(f"Filtering to {len(valid_targets)} holo_pdb IDs from {targets_csv.name}")
    else:
