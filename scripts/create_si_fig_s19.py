#!/usr/bin/env python3
"""
Legacy helper: PDB-manual target list (optional).

Not part of the default SI numbering; kept for a custom targets_PDB.csv workflow.
Writes figures/SF14_pdb.png when that CSV exists.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create SI Fig. S14 (Confusion Matrix Histograms for targets scraped manually from PDB)."
    )
    ...
    if panel_a.exists() and panel_b.exists():
        ...
        print(f"Collated figure saved to {collated_path.resolve()}")
    else:
