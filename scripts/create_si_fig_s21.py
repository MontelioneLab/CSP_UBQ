#!/usr/bin/env python3
"""
SI Fig. S21 — ideal N/H and HA/CA offsets (offset grid heatmaps from grid search).

Reads grid search CSV files from outputs/, creates the heatmaps, and saves to
./figures/SF21_ideal_offsets.png (N/H) and ./figures/SF21_ideal_offsets_ha_ca.png
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create SI Fig. S21 (ideal N/H and HA/CA offset heatmaps)."
    )
    ...
    print(
        f"[SF21] Aggregating ideal N/H offsets for n={len(keys)} targets "
        f"from {targets_csv.name} (resolved {len(selected_dir_names)} dirs)"
    )

    figures_dir.mkdir(parents=True, exist_ok=True)
    suppl_path = figures_dir / "SF21_ideal_offsets.png"
    ...
    if not heatmap_path:
        ...
        return 1
    print(f"SI Fig. S21 N/H panel saved to {suppl_path.resolve()} (n={len(keys)})")
    ...
    print(
        f"[SF21] Aggregating ideal HA/CA offsets for n={len(ha_ca_keys)} targets "
        f"from {targets_csv.name} (resolved {len(selected_dir_names)} dirs)"
    )

    ref = Referencing()
    ha_ca_path = figures_dir / "SF21_ideal_offsets_ha_ca.png"
