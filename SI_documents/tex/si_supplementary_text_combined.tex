...
$\mu = 26.2$\,ppm\,\AA{}$^{3}$ for benzene\textsuperscript{7}. The maximum absolute effect occurs
...
conditions\textsuperscript{9, 10}.
...
Using $|A|=21$\,ppm\,\AA{}$^{2}/e$ for backbone amide protons\textsuperscript{10}
...
from protein CSP measurements\textsuperscript{9, 10}. These values correspond
...
here\textsuperscript{10}. Although our apo and holo datasets were matched for
pH, ligand binding itself may alter protein ionization states and thereby
generate electric-field-induced CSPs.

\vspace{0.4em}
\noindent\textbf{Case Studies Figure Format}

Following the References, this document appends a concise case-study summary for
each of the 243 targets in the CSPdb. Each case study contains the following
panels: A)~a $^{15}$N/$^{1}$H HSQC scatterplot overlaying the apo and holo
assigned spectra with the optimal N/H offset applied; B)~Calculated CSPs for
receptor residues along the sequence, bars colored by confusion matrix
classification; C)~the mask of significant CSPs; D)~the mask of binding-site
residues; E)~the confusion matrix classification projected onto the structure:
...
binding site are blue (TN). Residues without CSP measurements are shown in gray;
ligands are colored cyan.
