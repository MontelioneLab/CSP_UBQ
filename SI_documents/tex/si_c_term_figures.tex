...
These results are very similar to CSPs calculated using SI Equation~S1. 71.9\% of CSPs using SI Equation~S3 are distant from the binding site compared to 69.4\% using SI Equation~S1.}
\end{figure}
...
These results are very similar to CSPs calculated using SI Equation~S1. 71.7\% of CSPs using SI Equation~S2 are distant from the binding site compared to 69.4\% using SI Equation~S1.}
\end{figure}
...
($n=121$ buffer-similar systems from \texttt{CSP\_UBQ\_ph0.5\_temp5C.csv}
with CA row coverage $>50\%$).
F1 scores on the $y$ axis use SI Equation~S1 (combined HN);
F1 scores on the $x$ axis use SI Equation~S2 (combined HN+C$\alpha$).
...
This figure shows there are no substantial differences in the distribution of F1 scores calculated based on 1D CSPs; distal CSPs occur for all types of atomic shifts at approximately the same frequency.}
\end{figure}
