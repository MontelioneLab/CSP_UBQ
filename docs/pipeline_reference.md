...
- Case-study figures are generated by default; use `--no-case-study` to skip. Use `--no-interactive-view` to skip figures that would require an interactive PyMOL capture.
...
- `--no-interactive-view`
  - Skip case-study figures that would require an interactive PyMOL view capture (F5). Reuse saved views under `pymol_views/` when present.
...
- `grid_h_min`, `grid_h_max`, `grid_h_step` (defaults **±0.2** ppm, step **0.01** ppm)  
- `grid_n_min`, `grid_n_max`, `grid_n_step` (defaults **±1.5** ppm, step **0.05** ppm)  
...
With `referencing_method="grid"` and `target_id` set, results cache to `<out>/<target_id>/offset_grid_<params>.csv` and heatmaps to `<out>/<target_id>/offset_grid_<params>.png` (where `<out>` is the active `--out` directory used by the pipeline run). Heatmaps place **¹H offset on the x-axis** and **¹⁵N offset on the y-axis**.

Override example:

```python
grid_params = {
    "h_min": -0.2, "h_max": 0.2, "h_step": 0.01,
    "n_min": -1.5, "n_max": 1.5, "n_step": 0.05,
    "cutoff": 0.05,
}
```
...
- `excluded_large_dn.csv` — residues skipped for `|ΔN_raw| > 15` ppm (when any)
- Aggregate across targets: `outputs/large_delta_n_exclusions.csv`  
- `occlusion_analysis.csv` — SASA occlusion details where applicable  
- `second_shell_filter.csv` — exclusive second-shell mask (`is_second_shell_6A`: receptor residues ≤6 Å any-atom from the binding-site union, excluding binding-site residues themselves)
- `master_alignment.csv` — merged per-residue table (includes `is_second_shell_6A` and `min_distance_to_binding_site`)
...
- `logs/09_second_shell_filter.txt` — exclusive second-shell (≤6 Å) filter
- `logs/10_tables_and_visualizations.txt` — table writing and visualization generation output
- `logs/11_master_csv.txt` — master CSV merge output
- `logs/12_case_study.txt` — case-study figure output (when enabled)
...
Residues with **raw** \(|\Delta\delta_N| > 15\) ppm (`Compute.max_abs_delta_n_ppm`) are excluded from CSP recording and significance (HN and H/N/CA paths). Shift fields are retained for traceability. The complete list of excluded apo/holo pairs and residues is written to `outputs/large_delta_n_exclusions.csv` (and per-target `excluded_large_dn.csv` on pipeline runs).
...
3. Cleaned-mean path cutoff: mean + `significance_z_score` × SD of the cleaned set (default **0.0** × SD → **mean** of cleaned CSPs).  
4. Primary HN cutoff floors that value at `cutoff_05_ppm` (default **0.05** ppm): `max(cleaned_mean + z·SD, 0.05)`.  

CLI overrides include `--outlier-z`, `--significance-z`, `--max-outlier-iterations`, `--max-outlier-fraction`, `--absolute-cutoff`.

Alternate boolean significance columns (cleaned sigma ladder without the primary
floor, raw-mean ladder, percentiles, and fixed ppm) are written to
`csp_table.csv` / `master_alignment.csv`. See
[significance_thresholds.md](significance_thresholds.md) for column names and
formulas.
