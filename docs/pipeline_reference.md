...
- In batch mode (no `--holo-pdb`), processing is serial because matplotlib is not thread-safe. If case-study generation is enabled in batch mode with `--workers > 1`, the pipeline warns and forces `--workers=1`.
- `--run-csv-metadata-annotation` is ignored when `--holo-pdb` is set.
- Detailed output now goes to per-target logs under `outputs/<holo_pdb>/logs/*.txt` (or `outputs/<holo_pdb>_<n>/logs/*.txt` for duplicate entries); stdout is intentionally reduced to step progress plus warnings/errors.
...
A run-level summary log is also written at `outputs/logs/confusion_matrix_summary.txt`.
...
3. **Inspect logs first** — check `outputs/<target>/logs/*.txt` for step-specific diagnostics  
