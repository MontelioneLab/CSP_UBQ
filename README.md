...
**Prerequisites:** populated `outputs/` (including per-target artifacts such as `master_alignment.csv`, and `confusion_matrix_per_system.csv` from the pipeline), and `data/CSP_UBQ_ph0.5_temp5C.csv` for one of the LaTeX table steps. If a sub-script file is missing, the wrapper logs and skips it (exit code 0 for that step). **PyMOL** is only required for interactive case-study figure capture when running the pipeline on a single target (case-study panels are generated by default unless you pass `--no-case-study`), not for this wrapper itself.
