...
**CSPdb** is a curated collection of protein–ligand systems for which paired apo and holo \(^{15}\text{N}\)-HSQC–derived assignments are available in the BMRB, together with holo coordinates in the PDB. This repository hosts the Python pipeline used to align sequences, reference and compare chemical shifts, define significant CSPs and binding-site residues, and summarize performance as a binary classification (true/false positives and negatives). The goal is to make CSP-based localization of interaction sites quantitatively comparable across many systems.
...
- H-bonds to the ligand (Baker–Hubbard criteria: \(\theta > 120^\circ\), H···A \(< 2.5\) Å).
- Charge complementarity with the ligand (opposite charged groups within 4.5 Å).
- \(\pi\) contacts (NH-to-aromatic within 6 Å).
...
## Regenerating figures and tables (wrapper script)

The script [scripts/create_all_supplement_figures_and_tables.py](scripts/create_all_supplement_figures_and_tables.py) runs the figure/table generators in sequence. Typical usage after a full pipeline run:
...
- `--include-placeholders` — also run scripts that are still placeholders or optional.
- `--stop-on-error` — exit on the first subprocess failure (default is to continue and report failures at the end).

**Prerequisites:** populated `outputs/` (including per-target artifacts such as `master_alignment.csv`, and `confusion_matrix_per_system.csv` from the pipeline), and `CSP_UBQ_ph0.5_temp5C.csv` in the repo root for one of the LaTeX table steps. If a sub-script file is missing, the wrapper logs and skips it (exit code 0 for that step). **PyMOL** is only required for interactive case-study figure capture when running the pipeline on a single target (case-study panels are generated by default unless you pass `--no-case-study`), not for this wrapper itself.
